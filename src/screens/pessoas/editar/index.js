...
export default () => {
    ...
    useFocusEffect(useCallback(() => {
        ...
        const response = async () => {
            ...
            if (userLogado) {
                const dados = await api.get(`/app/contact/show?contact=${route.params.id}`)

                setTipo_pessoa(dados.data.type == "supplier" ? "Juridica" : "Fisica")
                setCpf_cnpj(dados.data.cpf_cnpj)
                setRazao_social(dados.data.name)
                setInscricao_estadual(dados.data.ie_rg)
                setNome_fantasia(dados.data.supplier_business_name)
                setCep(dados.data.cep)
                setCidade(dados.data.cidade.nome)
                setCidadeIbge(dados.data.cidade.codigo)
                setBairro(dados.data.bairro)
                setRua(dados.data.rua)
                setNumero(dados.data.numero)
                setPonto_referencia(dados.data.landmark)
                setEmail(dados.data.email)
                setCelular(dados.data.mobile)
                setTelefone_alternativo(dados.data.alternate_number)
    ...
    const editar = async () => {
        ...
        try {
            setLoading(true)
            const response = await api.put(`/app/contact/edit?contact=${route.params.id}`, {
                ...
                {loadingScreen ? <ActivityIndicator color="#008be3" size={30} style={{ marginTop: 10 }} /> :
                    <ScrollView style={{ marginTop: 10, marginBottom: 20 }} showsVerticalScrollIndicator={false}>
                        <ContainerForm>
                            ...
                            <SelectDropdown
                                data={countries}
                                defaultButtonText={tipo_pessoa == "Juridica" ? "Juridica" : "Fisica"}
