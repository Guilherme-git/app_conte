...
export default () => {
    ...
    useFocusEffect(useCallback(() => {
        ...
        const response = async () => {
            ...
            if (userLogado) {
                ...
                const listClientes = await api.get(`/app/contact/list-customer?business_id=${userLogado.business.id}&owner_id=${userLogado.id}`)
    ...
    const abrirModalConfirmaPagamento = () => {
        if (produtosCarrinho.length > 0 && clienteSelecionado) {
            setDesconto(null)
            setModalConfirmarPagamento(true)
            setClienteSelecionadoNome(clientes.find(item => item.key == clienteSelecionado))
        } else {
                            ...
                            <>
                                <ContentDescription>
                                    <View style={{ marginBottom: 10 }}>
                                        <SelectList
                                            setSelected={(val) => setClienteSelecionado(val)}
                                            data={clientes}
                                            boxStyles={{ borderRadius: 8, borderWidth: 0, backgroundColor: '#f5f5f5', height: 40 }}
                                            placeholder={"Procure o clientes"}
                        ...
                        <Modal isVisible={modalConfirmarPagamento} onBackdropPress={fecharModalConfirmaPagamento}>
                            <ContentDescriptionModal>
                                ...
                                <ContentDescriptionHeader>
                                    ...
                                    <ContentDescriptionText>{clienteSelecionadoNome?.value} </ContentDescriptionText>
                                </ContentDescriptionHeader>
